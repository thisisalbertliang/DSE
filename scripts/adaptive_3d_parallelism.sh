...
gpt_options=" \
        ...
        --adaptive-3d-parallelism-interval 5
"
...
#  outputs=($(run_cmd))
  ...
  echo "ALBERT!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
  mp_size=$(head -1 new_mp_pp.txt)
  pp_size=$(tail -1 new_mp_pp.txt)
  echo "Running with model-parallel-size=$mp_size and pipe-parallel-size=$pp_size"
  gpt_options=" \
        --model-parallel-size ${mp_size} \
        --pipe-parallel-size ${pp_size} \
        --num-layers $NLAYERS \
        ...
        --fp16 \
        --tensorboard-dir ${LOGDIR}
        --adaptive-3d-parallelism-interval 20
  "
  full_options="${gpt_options} ${deepspeed_options} ${chkp_opt}"
  run_cmd="deepspeed --hostfile ${HOSTFILE} --num_nodes ${DLWS_NUM_WORKER} --num_gpus ${DLWS_NUM_GPU_PER_WORKER} pretrain_gpt2.py $@ ${full_options}"
done
